...
async function startAnalysis() {
    ...
    try {
        ...
        // Step 3: Detailed Fake/Genuine Analysis
        console.log('🔄 Starting Step 3: Detailed Analysis');
...
INSTRUKSI:
1. Pastikan backend berjalan di http://localhost:5001
...
async function loadSystemStatus() {
    try {
        ...
        if (backendAvailable) {
            ...
            backendElement.title = 'Backend API is running on http://localhost:5001';
        } else {
...
async function performDetailedAnalysis() {
    updateProgress(3, 'Melakukan analisis detail dengan 4 model AI...');

    try {
        if (!backendAvailable) {
            console.log('Backend not available, skipping detailed analysis');
            return;
        }

        const extractedText = analysisResults.ocrText || window.extractedText || '';
        const imageData = currentImageData || '';

        console.log('📊 Detailed analysis input:', {
            text_length: extractedText.length,
            has_image_data: !!imageData,
            text_preview: extractedText.substring(0, 100) + '...',
            image_data_available: currentImageData ? 'Yes' : 'No'
        });
        ...
        if (data.status === 'success' && data.data) {
            ...
            console.log('Detailed analysis completed:', data.data);
        } else {
            console.error('Detailed analysis failed:', data.error);
        }
...
function displayDetailedAnalysis() {
    ...
    const models = detailedAnalysis.models || {};
    const modelIcons = {
        'random_forest': 'fas fa-tree',
        'text_classifier': 'fas fa-font',
        'cnn': 'fas fa-brain',
        'ocr_confidence': 'fas fa-eye'
    };
    ...
    Object.entries(models).forEach(([modelKey, modelData]) => {
        ...
        const predictionClass = `prediction-${modelData.prediction}`;

        modelCard.innerHTML = `
            <div class="model-header">
                <div class="model-icon">
                    <i class="${modelIcons[modelKey] || 'fas fa-cog'}"></i>
                </div>
                <div class="model-name">${modelData.model_name || modelKey}</div>
            </div>
            <div class="model-prediction">
                <span class="prediction-badge ${predictionClass}">
                    ${modelData.prediction}
                </span>
                <span class="confidence-score">${modelData.confidence}%</span>
            </div>
            <div class="model-reasoning">
                <h4>Alasan Analisis:</h4>
                <ul class="reasoning-list">
                    ${(modelData.reasoning || []).map(reason => `<li>${reason}</li>`).join('')}
                </ul>
            </div>
        `;
    ...
    analysisSummary.innerHTML = `
        <div class="summary-title">
            Kesimpulan Analisis: ${detailedAnalysis.overall_prediction.toUpperCase()}
        </div>
        <div class="summary-content">
            <p><strong>Tingkat Kepercayaan:</strong> ${detailedAnalysis.overall_confidence}%</p>
            <p><strong>Penjelasan:</strong> ${detailedAnalysis.overall_reasoning}</p>
        </div>
    `;
